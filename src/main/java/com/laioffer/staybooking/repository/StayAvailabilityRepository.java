...
public interface StayAvailabilityRepository extends JpaRepository<StayAvailability, StayAvailabilityKey> {
    @Query(value = "SELECT sa.id.stay_id FROM StayAvailability sa " +
            "WHERE sa.id.stay_id IN ?1 AND sa.state = 0 AND sa.id.date BETWEEN ?2 AND ?3 " +
            "GROUP BY sa.id.stay_id HAVING COUNT(sa.id.date) = ?4")
    // ?2 or ?3 means the which No. in the parameter
